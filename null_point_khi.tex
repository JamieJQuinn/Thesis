...
\begin{figure}[t]
  ...
  \caption{\emph{Field configuration after $4$ Alfv\'en times.} The speed of the driver is also shown as a slice.}%
...
\begin{figure}[t]
  ...
  \includegraphics[width=1.0\linewidth]{param_study/mach_numbers_eta_4_swi.pdf}
    ...
    \end{subfigure}
  \caption{\emph{Plots of stability measures as functions of $r$ for all parameter choices at $t=8$.} Note the difference in the scale of $M_f$ for difference values of $\eta$. The cases where $\Lambda > 1$ are unstable with the exception of the isotropic cases where $\nu=10^{-4},\eta=10^{-3}$ (dashed line in figure~\ref{fig:mach_numbers_eta_3_iso}).}%
