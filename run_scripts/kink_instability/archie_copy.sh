...
for n in 3; do
  for m in 4; do
    ...
    for visc in '-isotropic'; do # '-switching'; do
      viscosity=$visc_mantissa'.0_num*10.0_num**(-'$n'_num)'
      resistivity=$resist_mantissa'.0_num*10.0_num**(-'$m'_num)'
      #folder=v0r5e-$m$visc
      #folder=v1e-${n}r0$visc
      #folder=anomalous-shock-v1e-${n}r1e-4$visc
      folder=anomalous-shock-v${visc_mantissa}e-${n}r${resist_mantissa}e-${m}$visc
      ...
      sed -i -e 's/eta0 =.*/eta0 = 1.0e-'$m'_num/' src/control.f90
      # Change size of box
