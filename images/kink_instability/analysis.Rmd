...
n_columns=2
n_rows=10
fig, axes = plt.subplots(n_rows, n_columns, figsize=(n_columns*8, (n_rows-1)*8), sharex=True, sharey=True)

for j in range(0, n_rows):
    plot = axes[j, 0].imshow(np.abs(isotropic_pef[j]), vmin=500, vmax=2000)
    axes[j,0].set_title(str(j))
#     plot = axes[j, 0].imshow(isotropic_pef[j])
    plot = axes[j, 1].imshow(np.abs(switching_pef[j]), vmin=500, vmax=2000)
