...
\begin{equation}
  {\left( \frac{\partial u}{\partial t}{x} \right)}_i = \frac{u_i - u_{i-1}}{dxb_i},
  \label{}
\end{equation}
and similarly we define the derivative of pressure (or any of the other cell-centred variables) at the boundaries,
\begin{equation}
  {\left( \frac{\partial p}{\partial t}{x} \right)}_i = \frac{p_i - p_{i-1}}{dxc_i}.
  \label{}
\end{equation}

\subsection{The Lagrangian Step}
The Lagrangian step is made up of a predictor-corrector scheme, a type of time-integration scheme that approximates an implicit scheme. Each timestep is split into two substeps, the first calculating an approximation of the pressure, and the second using this pressure to advance all other variables to the next full timestep. Here we start at timestep $n$, the predictor step advances the pressure to timestep $n+1/2$ and then finally the corrector step advances the remaining variables to timestep $n+1$.
\paragraph{The Predictor Step}
It can be shown by Taylor expansion that calculating $u^{n+1}$, $\varepsilon^{n+1}$ and $\rho^{n+1}$ to second order only requires calculating $p^{n+1/2}$ to first order. This is done using equation~\eqref{eqn-lare1d-equation-of-state},
\begin{equation}
...
Updating the variables during the Lagrangian step distorts the grid from its original state. The remap step rectifies this by mapping the variables from the distorted Lagrangian grid back to the original undeformed grid in a way that ensures conservation of mass. 

The remapping process requires no physics beyond the requirements that we conserve mass and energy, that is it is a purely geometrical exercise and the problem-specific physics are contained entirely in the Lagrangian step. This is advantageous when applying the scheme to a general set of problems since once the remap step is realised and implemented, the entire scheme can be adapted to a specific problem by changing only the equations in the Lagrangian step. It should also be noted that although the implementation detailed here takes one Lagrangian step per remap step, some LARE implementations take many Lagrangian steps, until the grid distortion becomes greater than some criteria, before a single remap step is taken. The following remap process could be used to remap a distortion created over multiple Lagrangian steps and only requires data from before and after the deformation of the grid. 

In order to successfully conserve mass we initially remap the density and then use the mass coordinate as a new measure of distance to remap the energy and velocity. 
...
\end{align}
where we have extracted  written $\psi = \bar{u}_i dt / dxb_i'$ TODO CHECK U BAR ISNT ABSOLUTE. The calculation of $\partial\rho_i'/ \partial x'$ is done with the use of flux limiters, discussed in section \ref{sec-flux-limiters}. Thus we can calculate the mass leaving the right hand side, 
\begin{equation}
...
\begin{equation}
  d\xi_i \frac{\partial \varepsilon_i'}{\partial t}{\xi} = dxb_i\rho_i \frac{\partial \varepsilon_i'}{\partial t}{\xi} = dxb_i \frac{\partial \varepsilon_i}{\partial t}{x}
\end{equation}
to rewrite $dE_i$ as
\begin{equation}
  dE_i = \left( \varepsilon_i' + \frac{1}{2}dxb_i\frac{\partial \varepsilon_i'}{\partial t}{x} \left( 1 - \frac{dM_i}{\rho_i dxb_i} \right) \right)dM_i,
...
\begin{align}
  dU_i &= \left( u_i' + \frac{1}{2}dxc_i\frac{\partial u_i'}{\partial t}{x} \left( 1 - \frac{dM_{i+1/2}}{\rho_{i+1/2} dxc_i} \right) \right)dM_{i+1/2},\\
...
\subsection{Limits of the Remap Step}
Looking at figure TODO DENSITY REMAP we can see a flaw in the remap step. What if the fluid moves so fast that the entire Eulerian cell is displaced? That is to say what if $\bar{u}_{i-1} dt > dxb_i$? The remap process would be unable to map the correct pieces of displaced mass to the Eulerian cells. A more robust (though much more complex) remap process could be constructed to handle this but a simpler solution would be to impose the limit that $\bar{u}_{i-1} dt < dxb_i$ over the entire grid. In practical tests this limit didn't appear to be breached, though I suspect small breaches of it would not cause solutions to explode, rather just lose mass erroneously. The limit could be breached in cases of very high fluid velocity or if too many Lagrangian steps were take per remap step.
...
This example of a LARE scheme was implemented in C++. This was due to the fact that it stradles well the boundary between speed and modern useability. The \emph{getopt} and \emph{json} libraries were used to allow input from a JSON file allowing for better flexibility. The unit testing library \emph{Catch} TODO REF was used to write some basic unit tests that greatly sped up any refactoring. Since this is a one-dimensional problem with limited complexity, there was no need to include parallelism in the code, though due to the lack of coupling between far apart grid points, it would a reasonably simple problem to parallelise.

Some helper tools were also written in Python and running scripts in bash. Due to the strong numerical and plotting libraries available for Python, it is a natural choice of language to write certain tools in, particularly tools for constructing initial condition data files and analysis tools. The iteration cycle is also extremely fast for Python, so writing quick and dirty tools is almost pleasant.
