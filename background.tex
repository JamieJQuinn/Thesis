...
In this chapter I introduce the physics required to understand the remainder of the thesis. To give context to the work I present an overview of the layers of the sun, including a  summary of recent developments in coronal heating. Next, I present the magnetohydrodynamic equations and a detailed introduction to the anisotropic nature of viscosity in a magnetised plasma.

\section{Introduction to solar physics}

\subsection{Layers of the Sun}

\todo{this section needs more references}

The stratified layers of the sun can be considered either as part of the solar interior, located below the photosphere, or as part of the overlying solar atmosphere. The photosphere, considered the surface of the sun, is located at a radius of $R_{\odot} = 695,508$km and emits radiation mostly in the visible spectrum.
...
\todo{sort out the following three paragraphs}
...
Additional non-ideal physics like ambipolar diffusion and the Hall effect can be modelled through additional terms in Ohm's law, however in the solar corona these effects are small compared to resistivity and are neglected as a result.
...
\end{equation}
This does not necessarily mean the magnetic field cannot impart energy to the fluid. When the fluid is resistive, the dissipation of currents can heat the fluid through a process called Joule or Ohmic heating. This is modelled in the energy equation by the term
\begin{equation}
...
\subsection{Viscosity in magnetised plasmas}
...
F_{+} = \frac{1}{2} \int^{x_0}_{x_0 - \Delta x} \frac{1}{\tau} m n u(x) \text{dx} = \frac{mn}{2} \left[ u(x_0) - \frac{\partial u}{\partial x} \frac{\Delta x}{2} \right] \frac{\Delta x}{\tau},
\end{equation}
...
In the absence of a magnetic field the viscosity is isotropic and particles are able to travel the full mean free path, hence $\Delta x = u \tau$, where $u$ is the thermal velocity. This gives an estimate for the isotropic viscous response,
\begin{equation}
...
\ten{\sigma}_{zz} \sim - \eta_0 \frac{\partial u_z}{\partial z}.
\end{equation}
...
\ten{\sigma}_{zx} \sim - \eta_{\perp} \frac{\partial u_z}{\partial x}, \quad \eta_{\perp} \sim \frac{\eta_0}{(\omega \tau)^2}.
\end{equation}
...
\ten{\sigma}_{xy} \sim - \eta_{\perp} \frac{\partial u_y}{\partial x}.
\end{equation}
...
\ten{\sigma}_{xx} = \ten{\sigma}_{yy} \sim - \eta_0 \nabla \cdot \vec{u}, \quad \ten{\sigma}_{xx} \sim \eta_0 \nabla \cdot \vec{u}.
\end{equation}
...
\end{equation}
Figure~\ref{fig:visc_dep_on_temp} shows the dependence of $\eta_0$ on temperature over a range of typical coronal temperatures, along with the dependence of a typical Reynolds number on temperature. \todo{fill this out}
